...
func _get_kb_action_name(action: StringName) -> String:
	return action + "@kb"
...
func is_action_just_pressed(action: StringName, device: int = KEYBOARD) -> bool:
		if not _is_device_controller(device): # KEYBOARD
			action = _get_kb_action_name(action)
		else: action = _get_joy_action_name(action, device)
		return Input.is_action_just_pressed(action)
		
		
func is_action_just_released(action: StringName, device: int = KEYBOARD) -> bool:
		if not _is_device_controller(device): # KEYBOARD
			action = _get_kb_action_name(action)
		else: action = _get_joy_action_name(action, device)
		return Input.is_action_just_released(action)
	
func is_action_pressed(action: StringName, device: int = KEYBOARD) -> bool:
		if not _is_device_controller(device): # KEYBOARD
			action = _get_kb_action_name(action)
		else: action = _get_joy_action_name(action, device)
...
func get_axis(negative: StringName, positive: StringName, device: int = KEYBOARD) -> float:
	if not _is_device_controller(device): # KEYBOARD
		negative = _get_kb_action_name(negative)
		positive = _get_kb_action_name(positive)
	else:
...
func get_vector(negative_x: StringName, positive_x: StringName,\
 negative_y: StringName, positive_y: StringName, device: int = KEYBOARD) -> Vector2:
	if not _is_device_controller(device): # KEYBOARD
		negative_x = _get_kb_action_name(negative_x)
		positive_x = _get_kb_action_name(positive_x)
		negative_y = _get_kb_action_name(negative_y)
		positive_y = _get_kb_action_name(positive_y)
	else:
